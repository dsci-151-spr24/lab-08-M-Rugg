...
evals %>%
  summarise(means = mean(score), medians = median(score), mins = min(score), maxs = max(score), sds = sd(score), q1 = quantile(score, probs = 0.25), q3 = quantile(score, probs = 0,75))
```

***Yes, the distribution is skewed to the low-end with most students ranking courses quite highly, between 4 and 5, and a median of 4.3. This mostly fits what I expected to see with the tail pointing towards the low-end as there are always some rough classes. However, I expected the bulk of the courses to be clustered towards 3-4 with less around 5 and was pleasantly surpried with how well the courses were rated.***
...
***There is a lot of variation; however, there does appear to be a weak, positive correlation between the average beauty rating of a professor and the average rating of the professor.***
...
***In the first graph, it was hidden (or misleading) how many points were actually stacked on top of one another. Since a scoring system is used most of the points will fall on exact (or the same rounded) numbers, and it masks how common an answer actually is.***
...
***For an average beauty increase of 1, the average evaluation rating increases by 0.07.***
...
***For an average beauty rating of 0, the average evaluation rating is 3.88. This doesn't make sense with the data since there are no professors with an average beauty rating of 0.***
